...
- **Security hardening — migration 0002 (applied):** `seed_org_defaults` EXECUTE revoked from `PUBLIC`/`anon`/`authenticated` and granted to `service_role` only; `handle_new_user` EXECUTE revoked from `PUBLIC`/`anon`/`authenticated` (trigger `on_auth_user_created` still fires). Verified: those two are no longer callable by anon/authenticated; the `auth_*` RLS helpers remain executable by `authenticated` (policies intact). Advisor warnings dropped 18 -> 14. Remaining 14 are the six `auth_*` helpers (required by RLS, low risk — read only `auth.uid()`) and `rls_auto_enable()` (pre-existing project event-trigger guardrail, not ours). A future migration could move the `auth_*` helpers to a non-exposed schema to clear the rest, but only with full policy updates + tests.
